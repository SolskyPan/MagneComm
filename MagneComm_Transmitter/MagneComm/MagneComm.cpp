...
DWORD preamble_pattern[6] = {0, 3, 1, 2, 4, 0 }; // 4 CPU core
...
void MagneComm::PreambleGenerator(DWORD window_length)
{
    ...
    for (int i = 0; i < CPU_CORE_TX_NUM; i++) {
        PWAMGenerator(window_length, width, preamble_pattern[i+1]);
    }
    // Set height as 0 
    Sleep(window_length);
}
...
void* MagneComm::HighSpeedTransmitControl(void* thread_arg)
{
    ...
    /*pwam_symbol_struct* thread_data;
    thread_data = (pwam_symbol_struct*)thread_arg;*/
    ...
    for (int i = 0; i < package_num; i++) {
        std::cout << "Current transmission package num is:" << i << std::endl;
        pwam_symbol_data = HighSpeedModulateData(symboldata[i]); //thread_data

        // Add the preamble on the head of the package
        for (int m = 0; m < 6; m++) {
            //std::cout << "Preamble symbol index is: " << m << std::endl;
            pthread_barrier_init(&SymbolBarrierLow, NULL, preamble_pattern[m] + 1);
            pthread_barrier_init(&SymbolBarrierHigh, NULL, preamble_pattern[m] + 1);

            for (int k = 0; k < preamble_pattern[m]; k++) {
                // start a thread
                pthread_create(&highspeed_threads[k], NULL, HighSpeedPWMGenerator, (void*)&cpu_affinity_mask[k]);
            }

            if ((m == 0) || (m == 5)) { //sleep(1) ms
                idleTime = HIGH_SPEED_PREAMBLE_PER_LENGTH;
                busyTime = 0;
            }
            else {
                busyTime = HIGH_SPEED_PREAMBLE_PER_LENGTH;
                idleTime = 0;
            }
            ...
            for (int k = 0; k < preamble_pattern[m]; k++) {
                // start a thread
                pthread_cancel(highspeed_threads[k]);
            }
            
            pthread_barrier_destroy(&SymbolBarrierLow);
            pthread_barrier_destroy(&SymbolBarrierHigh);

        }
        //std::cout << "Preamble Done!" << std::endl;
        //magnecomm->PreambleGenerator(1000);


        for (int j = 0; j < HIGH_SPEED_PACKAGE_SYMBOL_NUM_4BIT; j++) {
            //std::cout << "Current symbol num is:" << j << std::endl;
            /*std::cout << "Amplitude is :" << pwam_symbol_data[j].pulse_amplitude_value << std::endl;
            std::cout << "Width is :" << pwam_symbol_data[j].pulse_width_value << std::endl;*/
            
            pthread_barrier_init(&SymbolBarrierLow, NULL, pwam_symbol_data[j].pulse_amplitude_value + 1);
            pthread_barrier_init(&SymbolBarrierHigh, NULL, pwam_symbol_data[j].pulse_amplitude_value + 1);

            for (int k = 0; k < pwam_symbol_data[j].pulse_amplitude_value; k++) {
                // start a thread
                pthread_create(&highspeed_threads[k], NULL, HighSpeedPWMGenerator, (void*)&cpu_affinity_mask[k]);
            }
            ...
            //std::cout << "Transmit one bit done!" << std::endl;
            pthread_barrier_destroy(&SymbolBarrierLow);
            pthread_barrier_destroy(&SymbolBarrierHigh);
        }
    }
...
int MagneComm::HighSpeedTransmitData(char* file_path)
{
    ...
    if (!instream) {
        ...
        std::cout << "Total package number is: " << package_num << std::endl;
        symboldata = (int**)malloc(package_num * sizeof(int*));
        for (int i = 0; i < package_num; i++) {
        ...
        for (int i = 0; i < package_num; i++) {
            ...
            //pwam_symbol_data = HighSpeedModulateData(symboldata[i]);

            pthread_t thread_transmit;
            pthread_create(&thread_transmit, NULL, HighSpeedTransmitControl, (void*) NULL);
